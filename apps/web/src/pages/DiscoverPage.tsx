...
export function DiscoverPage(){
  ...
  useEffect(()=>{
    ...
    const poll=async()=>{
      try{
        ...
        if(runId)filters.set("runId",String(runId));
  ...
  return <>
    ...
    {task&&<TaskProgress task={task} onChange={refresh}/>} {deepTask&&<TaskProgress task={deepTask} onChange={refresh}/>}
    <section className="panel" id="discovery-results"><div className="panel-title"><div><span className="eyebrow">SAVED OPPORTUNITIES</span><h2>{results.counts.all} jobs collected</h2></div><button onClick={refresh}>Refresh saved results</button></div>
      ...
      <div className="discovery-filter-grid"><label>Search saved results<input value={params.get("query")||""} onChange={e=>filter("query",e.target.value)}/></label><label>Minimum score<select value={params.get("minScore")||"0"} onChange={e=>filter("minScore",e.target.value)}><option value="0">Any score</option><option value="40">40+</option><option value="60">60+</option><option value="75">75+</option></select></label><label>Analysis<select value={params.get("analysis")||""} onChange={e=>filter("analysis",e.target.value)}><option value="">Any analysis</option><option value="quick">Not deeply analyzed</option><option value="deep">Deeply analyzed</option></select></label></div>
