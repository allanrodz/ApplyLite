...
export function CvPage() {
  ...
  async function upload(event: React.FormEvent) {
    ...
    await operation(async () => { const body = new FormData(); body.append("file", file); show(await api<Draft>("/cv/upload", { method: "POST", body })); guideTo("cv-review"); setNotice("Source saved. Review the draft below; AI is optional."); });
  }
  ...
  async function save(event: React.FormEvent) {
    ...
    await operation(async () => {
      ...
      setCurrent(await api<CvDocument>("/cv/current")); guideTo("cv-merge"); setNotice("Reviewed facts saved for matching and application documents. Merge into Profile, then check your own target roles and locations.");
    });
  }
  return <>
    ...
    <section className="panel"><p>{current ? `Matching currently uses your last saved CV: ${current.sourceName}. Importing a draft does not replace it until you save reviewed facts.` : "No reviewed CV saved yet. Import and review a CV to provide factual evidence for matching and documents."}</p>
      {current && <button disabled={busy} onClick={() => void operation(async () => { show(await api<Draft>("/cv/drafts/from-current", { method: "POST", body: "{}" })); setNotice("Existing CV opened as an editable draft."); guideTo("cv-review"); })}>Edit the saved CV</button>}</section>
    ...
    {draft && facts && <>
      ...
      {draft.diagnostics && draft.diagnostics.missingFields.length > 0 && <section className="panel" aria-label="CV completeness"><h2>Recommended details to review</h2><p>These do not block browsing. Dates improve experience scoring; leave unknown values blank.</p>{draft.diagnostics.missingFields.map(m=><p key={m.path}><button type="button" onClick={()=>guideTo(`cv-field-${m.path.replaceAll(".","-")}`)}>{m.label}</button> {m.message}</p>)}</section>}
      <form className="panel" id="cv-review" tabIndex={-1} onSubmit={save}><h2>Review and edit facts</h2><p>Leave unknown values empty. Saving confirms your review and makes this CV available to matching and document generation.</p><div className="form-grid">
        {(["fullName", "email", "phone", "headline", "summary", "city", "country", "linkedinUrl", "githubUrl", "portfolioUrl"] as const).map(key => <label key={key} className={key === "summary" ? "full" : ""}>{title(key)}{key === "summary" ? <textarea rows={4} value={facts[key]} onChange={e => setScalar(key, e.target.value)} /> : <input id={`cv-field-${key}`} aria-invalid={draft.diagnostics?.missingFields.some(m=>m.path===key)||undefined} value={facts[key]} onChange={e => setScalar(key, e.target.value)} />}</label>)}
