...
type Handler={run:(input:any,context:TaskContext)=>Promise<unknown>;settled?:(input:any,task:Task)=>void};
...
async function pump(){
 ...
 const run=async()=>{
  ...
  finally{clearInterval(timer);notify(candidate.id);active=null;schedule();}
 };
...
export function retryTask(id:string){const old=row(id);if(!old)throw new Error("Task not found.");if(!["FAILED","CANCELLED","INTERRUPTED"].includes(old.status))throw new Error("Only failed, cancelled or interrupted tasks can be retried.");return enqueueTask(old.kind,old.subject_key,JSON.parse(old.input_json),id);}
