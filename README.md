...
15. Skill AI is user-led: clicking **AI** opens the chat without generating anything. Ask the first question yourself. The backend requests a small structured final answer, suppresses model reasoning tags before display, and uses a larger bounded output budget to reduce truncated-answer failures. ZeroGPT human-verification checks are visibility-aware, so hidden challenge widgets no longer cause false HTTP 409 conflicts; a genuinely visible challenge is shown as a normal in-app notice.
